...
*Questa è la traduzione italiana di [DOCS.md](DOCS.md), la versione di
riferimento mostrata nel tab "Documentazione" dell'add-on.*
...
sovrascritte a ogni riavvio — con un'eccezione: le impostazioni DNS e le
restrizioni OIDC, che sono di proprietà di Headplane o dell'utente e
vengono preservate (vedi "DNS e restrizioni OIDC" più sotto).
...
| Impostazioni DNS (nameserver, split DNS, override, base domain, MagicDNS) e restrizioni OIDC | Riavvio automatico del solo servizio headscale entro ~10 secondi; subito dopo si riavvia anche la dashboard (per riagganciare il nuovo processo). L'add-on resta su e i client si riconnettono da soli |
